...
unsafe partial class App {
    static bool ShowFourierTransform(App app,
        ...
        var wav = new Source();
        var X = Process.Sine(440,
            wav.Hz,
            1024);
        wav.Push(Complex.FFT(X));
        app.StartWinUI<ISource>(
            onDrawFourierTransform, () => wav, "Fast Fourier Transform (Mic)",
            Color.Red,
            null);
        return false;
    }

    public static void onDrawFourierTransform(Surface2D Canvas, float phase, ISource wav) {
        int hz = wav?.Hz ?? 0;
